...
etsmodel <-
function(y, errortype, trendtype, seasontype, damped,
...
function(par, y, nstate, errortype, trendtype, seasontype, damped, par.noopt, lowerb, upperb,
...
initparam <-
function(alpha, beta, gamma, phi, trendtype, seasontype, damped, lower, upper, m) {
  if (any(lower > upper)) {
...
check.param <-
function(alpha, beta, gamma, phi, lower, upper, bounds, m) {
...
initstate <-
function(y, trendtype, seasontype) {
  if (seasontype != "N") {
    # Do decomposition
    m <- frequency(y)
    n <- length(y)
...
pegelsresid.C <-
function(y, m, init.state, errortype, trendtype, seasontype, damped, alpha, beta, gamma, phi, nmse) {
  n <- length(y)
  p <- length(init.state)
  x <- numeric(p * (n + 1))
  x[1:p] <- init.state
...
admissible <-
function(alpha, beta, gamma, phi, m) {
  if (is.null(phi)) {
    phi <- 1
  }
  if (phi < 0 || phi > 1 + 1e-8) {
